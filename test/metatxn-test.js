...
describe("MetaTokenTransfer", function () {
    it("Should let user transfer tokens through a relayer with different nonces", async function () {
        ...
        // one as the relayer address and one as the receipient address
        ...
        // Have user infinite approve the token sender contract for transfering 'RandomToken'
        const approveTxn = await userTokenContractInstance.approve(tokenSenderContract.target,
            BigInt(
                ...
                // 64 digits = 32bytes
                // 32 bytes = 256bits = uint256
                "0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
            ));
            await approveTxn.wait();

            // Have user sign message to transfer 10 tokens to receipient
            let nonce = 1;
            ...
            let recipientBalance = await randomTokenContract.balanceOf(receipientAddress.address);
            ...
            // Check wheher user's balance decreased and recipient got 10 tokens
    ...
    it("Should not let signature replay happen", async function () {
        ...
        // Get three addresses, treat one as the user address, one as the relayer address, one as the recipient address
        const [_, userAddress, relayerAddress, recipientAddress] = await ethers.getSigners();
        ...
        // Have user infinite approve the token sendercontract for transfering `RandomToken`
